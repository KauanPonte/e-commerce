<template>
  <PrimeToast />

  <div class="min-h-screen flex">
    <!-- Painel esquerdo -->
    <div class="hidden lg:flex w-1/2 bg-blue-700 flex-col items-center justify-center px-16 gap-6">
      <RouterLink to="/" class="text-white text-4xl font-bold tracking-tight">
        ShopFruit
      </RouterLink>
      <p class="text-blue-200 text-center text-lg leading-relaxed max-w-xs">
        Crie sua conta e aproveite as melhores ofertas de frutas frescas.
      </p>
      <div class="flex gap-2 mt-4">
        <span class="w-2 h-2 rounded-full bg-yellow-400"></span>
        <span class="w-2 h-2 rounded-full bg-blue-400"></span>
        <span class="w-2 h-2 rounded-full bg-white opacity-40"></span>
      </div>
    </div>

    <!-- Painel direito -->
    <div class="flex-1 flex items-center justify-center bg-gray-50 px-6 py-12">
      <div class="w-full max-w-sm">
        <RouterLink to="/" class="lg:hidden block text-blue-700 text-2xl font-bold mb-8 text-center">
          ShopFruit
        </RouterLink>

        <h1 class="text-2xl font-bold text-gray-800 mb-1">Criar conta</h1>
        <p class="text-gray-500 text-sm mb-8">Preencha os dados abaixo para se registrar</p>

        <form @submit.prevent="submit" class="flex flex-col gap-5">
          <!-- Nome -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-gray-700">Nome</label>
            <InputText
              v-model="form.name"
              placeholder="Seu nome completo"
              :invalid="v$.name.$error"
              class="w-full"
            />
            <span v-if="v$.name.$error" class="text-red-500 text-xs">
              {{ v$.name.$errors[0].$message }}
            </span>
          </div>

          <!-- E-mail -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-gray-700">E-mail</label>
            <InputText
              v-model="form.email"
              placeholder="seu@email.com"
              :invalid="v$.email.$error"
              class="w-full"
            />
            <span v-if="v$.email.$error" class="text-red-500 text-xs">
              {{ v$.email.$errors[0].$message }}
            </span>
          </div>

          <!-- Senha -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-gray-700">Senha</label>
            <PrimePassword
              v-model="form.password"
              placeholder="Mínimo 6 caracteres"
              :feedback="false"
              toggleMask
              :invalid="v$.password.$error"
              class="w-full"
              fluid
            />
            <span v-if="v$.password.$error" class="text-red-500 text-xs">
              {{ v$.password.$errors[0].$message }}
            </span>
          </div>

          <!-- Confirmar Senha -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-gray-700">Confirmar Senha</label>
            <PrimePassword
              v-model="form.confirmPassword"
              placeholder="Repita a senha"
              :feedback="false"
              toggleMask
              :invalid="v$.confirmPassword.$error"
              class="w-full"
              fluid
            />
            <span v-if="v$.confirmPassword.$error" class="text-red-500 text-xs">
              {{ v$.confirmPassword.$errors[0].$message }}
            </span>
          </div>

          <button
            type="submit"
            :disabled="loading"
            class="w-full mt-1 py-2.5 rounded-lg bg-yellow-400 hover:bg-yellow-500 disabled:opacity-60 text-gray-800 font-semibold text-sm transition-colors flex items-center justify-center gap-2"
          >
            <i v-if="loading" class="pi pi-spin pi-spinner" />
            {{ loading ? 'Criando conta...' : 'Criar conta' }}
          </button>
        </form>

        <p class="mt-6 text-center text-sm text-gray-500">
          Já tem uma conta?
          <RouterLink to="/login" class="text-blue-700 font-semibold hover:underline ml-1">
            Entrar
          </RouterLink>
        </p>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { reactive, ref } from 'vue'
import { useVuelidate } from '@vuelidate/core'
import { required, email, minLength, helpers } from '@vuelidate/validators'
import { useRouter } from 'vue-router'
import { useToast } from 'primevue/usetoast'
import { useAuthStore } from '@/stores/auth.store'

const authStore = useAuthStore()
const router = useRouter()
const toast = useToast()
const loading = ref(false)

const form = reactive({ name: '', email: '', password: '', confirmPassword: '' })

const rules = {
  name: {
    required: helpers.withMessage('Nome é obrigatório', required),
  },
  email: {
    required: helpers.withMessage('E-mail é obrigatório', required),
    email: helpers.withMessage('Formato de e-mail inválido', email),
  },
  password: {
    required: helpers.withMessage('Senha é obrigatória', required),
    minLength: helpers.withMessage('A senha deve ter no mínimo 6 caracteres', minLength(6)),
  },
  confirmPassword: {
    required: helpers.withMessage('Confirme a senha', required),
    // Validação customizada: verifica se os dois campos de senha são iguais
    sameAs: helpers.withMessage(
      'As senhas não coincidem',
      () => form.confirmPassword === form.password,
    ),
  },
}

const v$ = useVuelidate(rules, form)

async function submit() {
  const valid = await v$.value.$validate()
  if (!valid) return

  loading.value = true
  try {
    await authStore.register(form.name, form.email, form.password)
    toast.add({ severity: 'success', summary: 'Conta criada com sucesso!', life: 2000 })
    setTimeout(() => router.push('/'), 700)
  } catch {
    toast.add({
      severity: 'error',
      summary: 'Erro ao criar conta',
      detail: 'Tente novamente mais tarde',
      life: 4000,
    })
  } finally {
    loading.value = false
  }
}
</script>
